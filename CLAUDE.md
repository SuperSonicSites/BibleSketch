...
- The scheduled task `pinterest-daily` (7:00 local, desktop app) is owner-authorized (2026-09-24) to generate, review, publish and schedule 5 Pins a day and to deploy the Worker and push `pins.json` plus the runbook, within the scope written in the runbook's "Daily scheduled task" section. That standing approval covers only that task's runs.
