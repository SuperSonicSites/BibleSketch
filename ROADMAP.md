...
- **S8. Story workbooks ("show and tell").** Owner idea 2026-09-25: tell a whole story (first: Noah) as a printable workbook where every spread has the Scripture on one side and its coloring page on the other, so a child can retell the story page by page. Fits what exists: the Worker already builds print PDFs with `pdf-lib` (`web/src/pages/api/print/[id].ts`), so a workbook is the same code over several sketches (left page: passage text, reference and a few writing lines such as "What is Noah doing?"; right page: the drawing), plus a cover. We already have reviewed Noah pages for Pinterest (Genesis 7:1, 7:8-9, 8:18-19), so the first workbook can be assembled by hand from existing sketches before any generator work. Later: a "Make a story workbook" flow that takes a passage range (Genesis 6-9), splits it into 6-10 scenes with the brief model, generates each scene in one style and age, and charges per page. Open for the owner: free lead magnet, Premium perk or paid; Bible translation for the text pages (Verse Art uses WEB, which is public domain); fixed curated stories vs user-built; whether each spread is a two-page layout or one page split in half (easier to print single-sided).
