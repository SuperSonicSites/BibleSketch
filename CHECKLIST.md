...
- [ ] **Resend account.**
  ...
  - [ ] **Approve deploying the billing webhook fix** (`firebase deploy --only "functions:handleZohoWebhook"`). Only the Premium plan code can grant Premium now. Deploy it before creating any new plan in Zoho (e.g. the $1.99 prints plan, plan §12.20), then give Claude that plan's code.
