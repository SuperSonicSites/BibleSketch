...
- [ ] **Pinterest API app** "Bible Sketch Pin Publisher" (id 1615048, Trial access since 2026-09-23; redirect URI `https://biblesketch.app/api/pinterest/callback` registered). Owner steps: (1) in `web/`, `npx wrangler secret put PINTEREST_APP_SECRET` and paste the app secret from developers.pinterest.com > My apps > Manage (never share it); (2) open https://biblesketch.app/api/pinterest, Connect, approve on Pinterest, publish one Sandbox Pin; (3) screen-record steps 2 end to end (about a minute) and request the Standard upgrade (My apps > Upgrade) with that video. After Standard: `PINTEREST_ENV` → `production` in `web/wrangler.jsonc`, reconnect, then the daily API publisher replaces the RSS feeds (alt text and AI disclosure included).
