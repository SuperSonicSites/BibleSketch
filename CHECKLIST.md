...
0a. **Zoho API client for the on-site USD checkout** (plan §12.20 item 4). Claude finished the Zoho Billing side in your browser on Sep 25 (see the plan). What's left:
   1. Open https://api-console.zohocloud.ca, choose **Self Client** (create it if there's none), and copy its Client ID and Client Secret.
   2. Set them yourself: `firebase functions:secrets:set ZOHO_CLIENT_ID --project biblesketch-5104c`, then the same for `ZOHO_CLIENT_SECRET`.
   3. In the Self Client's **Generate Code** tab, enter the scope `ZohoSubscriptions.customers.CREATE,ZohoSubscriptions.hostedpages.CREATE`, choose 10 minutes, and create it. Within 10 minutes, run Claude's exchange command. It uses `accounts.zohocloud.ca` and stores `ZOHO_REFRESH_TOKEN` directly.
