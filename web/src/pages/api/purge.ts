...
// Drops every cached response tagged sketch:<id> (page, slug redirects, 404) from Workers Cache, globally.
import type { APIRoute } from 'astro';
import { env } from 'cloudflare:workers';

const MAX_IDS = 100; // Workers Cache purge takes up to 100 tags per call
...
export const POST: APIRoute = async ({ request, cache }) => {
  ...
  if (ids.length === 0 || ids.length > MAX_IDS) return new Response('ids: 1-100 strings', { status: 400 });
  try {
    await cache.invalidate({ tags: ids.map((id) => `sketch:${id}`) });
  } catch (e) {
